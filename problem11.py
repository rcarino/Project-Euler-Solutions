...
m = ['08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08'.split(' '),
...
def sum_coords(c1, c2):
    return tuple(map(sum, zip(c1, c2)))


directions = [[(0, 0), (-1, 0), (-2, 0), (-3, 0)], # Up
              [(0, 0), (1, 0), (2, 0), (3, 0)], # Down
              [(0, 0), (0, -1), (0, -2), (0, -3)], # Left
              [(0, 0), (0, 1), (0, 2), (0, 3)], # Right
]

directions.append(map(sum_coords, directions[0], directions[2]))  # Up left
directions.append(map(sum_coords, directions[0], directions[3]))  # Up right
directions.append(map(sum_coords, directions[1], directions[2]))  # Down left
directions.append(map(sum_coords, directions[1], directions[3]))  # Down right
    ...
    for col in range(len(m)):
        for direction in directions:
            ...
            for coords in direction:
                x,y = coords
                if 0 <= row + x < len(m) and 0 <= col + y < len(m):
                    dir_val *= int(m[row + x][col + y])
            if dir_val > max_prod:
                max_prod = dir_val
                max_start = (row, col)
